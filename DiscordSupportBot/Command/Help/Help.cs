...
namespace DiscordSupportBot.Command.Help
{
    public class Help : TopLevelModule<HelpService>
    {
        ...
        public async Task Modules()
        {
            await ReplyAsync("", false, new EmbedBuilder().WithOkColor().WithTitle("模組清單")
                ...
                .WithFooter("輸入 `!!!Commands 模組名稱` 以顯示模組內全部的指令，例 `!!Commands Help`")
                .Build());
        }
        ...
        public async Task Commands([Summary("模組名稱")] string module = null)
        {
            ...
            if (cmds.Count() == 0) { await ReplyAsync($"找不到 {module} 模組"); return; }
            ...
            if (cmds.Count() == 0)
            {
                await ReplyAsync(module + " 未包含你可使用的指令");
                return;
            }

            var embed = new EmbedBuilder().WithOkColor().WithTitle($"{cmds.First().Module.Name} 內包含的指令").WithFooter("輸入 `!!!Help 指令` 以顯示指令的詳細說明，例 `s!Help Help`");
        ...
        public async Task H([Summary("指令名稱")] string command = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(command))
            {
                ...
                embed.Title = "輔助小幫手 建置版本" + Program.VERSION;
#if DEBUG
                embed.Title += " (測試版)";
#endif
